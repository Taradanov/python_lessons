...
class Sklad:
    ...
    def move_position(self, to_sklad, sku, amount):
        if self.check_available(sku, amount):
            ...
            return False, f'Недостаточно товара {sku}'
...
class Interface:
    ...
    @staticmethod
    def input_sklad():
        return Sklad(Interface.input_string('Введите имя склада'))

    @staticmethod
    def input_position(cls):
        return cls(
            Interface.input_string('Введите марку'),
            Interface.input_string('Введите код поставщика'),
            Interface.input_string('Введите описание')
        )


sklad = Interface.input_sklad()
office = Interface.input_sklad()

copier = Interface.input_position(Copier)
printer = Interface.input_position(Printer)
